...
let operator;
// Array to store valid operators
const operatorArray = ["÷", "x", "-", "+"];
// Array to store numbers and operators in order
...
const operation = (num1, num2, operator) => {
    // Functions for the separate operations
    const add = (a, b) => a + b;
...
/* Update operator variable each time the operator button is clicked */
...
const writeToDisplay = (item) => {
    ...
    /* Delete the previous operator if there is one so no multiple operators exist */
    if (operatorArray.includes(item) && operatorArray.includes(lastItemAtDisplay)) {
...
function orderofOperations() {
    ...
    while (numbersAndOperationsArray.includes("+") || numbersAndOperationsArray.includes("-")) {
        // if left to right, addition first do this
        if (numbersAndOperationsArray.indexOf("+") > numbersAndOperationsArray.indexOf("-")) {
            ...
            numbersAndOperationsArray.splice(xIndex - 1, 2);
            // if left to right, subtraction is first do this
